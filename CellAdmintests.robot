...
CELLS WHEN DOMAIN IS SELECTED
	...
	# Selecting a domain triggers an AJAX event. We need to wait until the cell list is available again (<=10ms)
	${CELLS_LIST}=	WAIT UNTIL KEYWORD SUCCEEDS	10s	10ms	GET LIST ITEMS	${CELLS_LIST_ELEMID}
	${NUMBER_Of_CELLS}=	GET LENGTH	${CELLS_LIST}
	SHOULD BE TRUE		${NUMBER_OF_CELLS} > 1
	CLOSE BROWSER
